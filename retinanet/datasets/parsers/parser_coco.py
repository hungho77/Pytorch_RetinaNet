...
class CocoParser(Parser):
    ...
    def _load_annotations(self, ann_file):
        ...
        self.cat_ids = self.coco.getCatIds() # type: ignore
        self.cat_names = [c['name'] for c in self.coco.loadCats(ids=self.cat_ids)]
        if not self.cat_ids_as_labels:
            self.cat_id_to_label = {cat_id: i + self.label_offset for i, cat_id in enumerate(self.cat_ids)} # type: ignore
        ...
        for img_id in sorted(self.coco.imgs.keys()):
            ...
            if (min(info['width'], info['height']) < self.min_img_size or
                ...
                self.img_ids_invalid.append(img_id) # type: ignore
                continue
            self.img_ids.append(img_id) # type: ignore
            self.img_infos.append(info) # type: ignore
    ...
    def _parse_img_ann(self, img_id):
        ...
        for i, ann in enumerate(ann_info):
            ...
            if ann.get('iscrowd', False):
                ...
                bboxes.append(bbox)
                cls.append(self.cat_id_to_label[ann['category_id']] if self.cat_id_to_label else ann['category_id']) # type: ignore
        ...
        if self.include_bboxes_ignore:
            ann['bbox_ignore'] = bboxes_ignore # type: ignore
